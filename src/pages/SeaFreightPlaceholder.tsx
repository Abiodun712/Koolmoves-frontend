...
function normalizeSeaGood(
  ...
  const packingRequested =
    requestedIds.has(String(row.id)) || String(row.status || '').toLowerCase() === 'requested';
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Sea goods for KM-ID <span className="font-bold text-gray-700">{kmId}</span>. Add
                ...
                {availableGoods.length === 0 ? (
                  <div className="p-4 rounded-xl border border-dashed border-gray-300 bg-slate-50 text-xs text-gray-600">
                    {unrequestedGoods.length === 0
                      ? 'No Sea goods are available for a new packing request.'
                    ...
                    >
                      ...
                      {item.photo_url ? (
                        <img
                          ...
                          className="w-full max-w-full max-h-36 rounded-xl border border-gray-200 object-cover"
                        />
                      ) : null}
                    </div>
                  ))
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 shadow-sm space-y-4 w-full min-w-0">
          ...
          <label className="block w-full space-y-1 min-w-0">
            ...
            <textarea
              ...
              className="w-full text-xs px-3 py-2.5 rounded-xl border border-gray-200 bg-white text-gray-800 disabled:bg-gray-100 resize-y"
            />
          </label>
          <p className="text-xs text-gray-600">
            {selectedCount === 0
              ? 'Select one or more Sea goods, then choose a Nigeria pickup warehouse.'
                ...
                >
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-2">
                      ...
                      <div className="grid grid-cols-5 gap-0.5 sm:gap-1 min-w-0">
                      {SEA_USER_PROGRESS_STEPS.map((step, index) => {
                        ...
                        return (
                          <div key={step.status} className="min-w-0">
                            ...
                            />
                              <p
                                className={`mt-1 text-[8px] sm:text-[10px] font-bold leading-tight break-words ${
                                  current
                                    ? 'text-emerald-800'
                                    : reached
                                      ? 'text-emerald-700'
